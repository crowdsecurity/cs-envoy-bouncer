...
build-filter:
	...
	cd filter && cargo build --target $(RUST_TARGET) --release

.PHONY: build-updater
build-updater:
	@echo "Building updater..."
	cd updater && cargo build --target $(RUST_TARGET) --release
...
test:
	...
	@echo "  cd filter && cargo test config::tests"
	@echo "  cd updater && cargo test config::tests"

.PHONY: clean
clean:
	@echo "Cleaning..."
	cd filter && cargo clean
	cd updater && cargo clean
	rm -f *.tar.gz
...
tarball: build
	...
	@cp filter/target/$(RUST_TARGET)/release/crowdsec_filter.wasm crowdsec-envoy-bouncer-$(BUILD_VERSION)/
	@cp updater/target/$(RUST_TARGET)/release/crowdsec_updater.wasm crowdsec-envoy-bouncer-$(BUILD_VERSION)/
...
lint:
	@echo "Running lint..."
	cd filter && cargo clippy -- -D warnings
	cd updater && cargo clippy -- -D warnings

.PHONY: vendor
vendor:
	@echo "Vendoring dependencies..."
	cd filter && cargo vendor
	cd updater && cargo vendor
